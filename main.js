const sectionDefinitions = [
  ...
  { id: "experience", label: "Learning", title: "Learning Log", command: "tail learning.log" },
  { id: "skills", label: "Skills", title: "Skills Matrix", command: "ls stack/" },
  { id: "projects", label: "Practice", title: "Practice Notes", command: "open practice/" },
  { id: "education", label: "Education", title: "Education Archive", command: "cat education.txt" },
  { id: "certifications", label: "Certs", title: "Certifications", command: "cat certs.txt" },
...
function renderCertifications(data) {
  const items = (data.certifications || [])
    .map(
      (item) => `
        <article class="education-card">
          <h3 class="education-title">${escapeHtml(item.name)}</h3>
          <p class="education-meta">${escapeHtml(item.issuer)}</p>
...
function buildGmailComposeUrl(email) {
  const subject = "CEH Internship Opportunity";
  const body = [
    "Hello Shaurya,",
    "",
    "I reviewed your cybersecurity internship profile and would be glad to connect.",
    "",
    "Let us discuss possible CEH internship or beginner cybersecurity opportunities.",
    "",
...
function resolveCommand(command) {
  ...
  if (normalized === "ls practice" || normalized === "ls practice/") {
    return { type: "ls-projects" };
  }
  ...
  if (normalized === "cat skills.txt" || normalized === "skills") {
    return { type: "cat-skills" };
  }

  if (normalized === "cat certs.txt" || normalized === "cat certifications.txt" || normalized === "certs") {
    return { type: "cat-certs" };
  }

  if (normalized === "cat owasp.txt" || normalized === "owasp") {
    return { type: "cat-owasp" };
  }

  if (normalized === "cat ctf.txt" || normalized === "ctf" || normalized === "picoctf") {
    return { type: "cat-ctf" };
  }
...
async function initializeTerminal(data) {
  ...
  async function runCommand(rawCommand) {
    ...
    if (resolved.type === "help") {
      ...
      addOutputLine(output, `<span class="output-inline">whoami</span>  <span class="output-inline">pwd</span>  <span class="output-inline">ls</span>  <span class="output-inline">ls practice/</span>`, "system");
      addOutputLine(output, `<span class="output-inline">cat intro.txt</span>  <span class="output-inline">cat skills.txt</span>  <span class="output-inline">cat certs.txt</span>  <span class="output-inline">cat owasp.txt</span>  <span class="output-inline">cat ctf.txt</span>`, "system");
      addOutputLine(output, `<span class="output-inline">cat contact.txt</span>  <span class="output-inline">cat reactions.log</span>`, "system");
    ...
    if (resolved.type === "ls-root") {
      addOutputLine(
        output,
        formatInlineList(["intro.txt", "skills.txt", "certs.txt", "owasp.txt", "ctf.txt", "practice/", "contact.txt", "reactions.log"]),
        "system"
      );
      return;
    }
    ...
    if (resolved.type === "cat-skills") {
      (data.skills || []).forEach((group) => {
        addOutputLine(output, `<span class="output-inline">${escapeHtml(group.category)}</span>`, "success");
        addOutputLine(
          output,
          formatInlineList(group.items || []),
          "system"
        );
      });
      return;
    ...
    if (resolved.type === "cat-certs") {
      (data.certifications || []).forEach((cert) => {
        addOutputLine(
          output,
          `<span class="output-inline">${escapeHtml(cert.name)}</span> - ${escapeHtml(cert.issuer)}. ${escapeHtml(cert.detail || "")}`,
          "system"
        );
      });
      return;
    ...
    if (resolved.type === "cat-owasp") {
      addOutputLine(output, "Studied OWASP Top 10 as a web security foundation.", "success");
      addOutputLine(output, formatInlineList(["Injection", "Broken Access Control", "Authentication Risks", "Security Misconfiguration", "Vulnerable Components"]), "system");
      return;
    }

    if (resolved.type === "cat-ctf") {
      addOutputLine(output, "picoCTF practice completed as hands-on cybersecurity learning.", "success");
      addOutputLine(output, "Focus areas: Linux commands, problem solving, security basics, and beginner CTF methodology.", "system");
      return;
    }
    ...
    if (resolved.type === "contact") {
      ...
      addOutputLine(
        output,
        `Prefilled subject: <span class="output-inline">CEH Internship Opportunity</span>`,
        "system"
      );
      return;
    }
